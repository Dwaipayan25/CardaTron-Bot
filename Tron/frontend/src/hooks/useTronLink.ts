...
export function useTronLink() {
  ...
  const updateBalance = useCallback(async (address: string) => {
    ...
    try {
      ...
      let usdcBalance = 0
      try {
        console.log("📋 Loading USDC contract for balance check...");
        // Use the TronWeb contract pattern like in the minting script
        const usdcContract = await window.tronWeb.contract().at(NETWORKS.tron.usdc)
        
        console.log("🔍 Checking USDC contract details...");
        const [symbol, decimals] = await Promise.all([
          usdcContract.symbol().call(),
          usdcContract.decimals().call()
        ]);
        ...
        console.log("📊 Checking user's current USDC balance...");
        const addressHex = window.tronWeb.address.toHex(address)
        const currentBalance = await usdcContract.balanceOf(addressHex).call()
        
        // Handle BigInt balance properly - same as minting script
        const decimalsNumber = Number(decimals);
        const currentBalanceFormatted = Number(currentBalance) / Math.pow(10, decimalsNumber);
        console.log(`  Current USDC Balance: ${currentBalanceFormatted.toLocaleString()} ${symbol}`);
        
        usdcBalance = currentBalanceFormatted
      } catch (err) {
  ...
  const sendUSDC = useCallback(async (to: string, amount: string) => {
    ...
    try {
      // Use the correct deployed USDC contract address from config
      const usdcContract = await window.tronWeb.contract().at(NETWORKS.tron.usdc)
