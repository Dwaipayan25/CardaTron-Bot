...
const generateOrderSteps = (order: Order) => {
  ...
  // Toggle 'Order Completed' to completed if all previous steps are completed
  const allPrevCompleted = steps.slice(0, 6).every(step => step.status === "completed");
  if (allPrevCompleted) {
...
export function OrderDetail({ orderId, onBack }: OrderDetailProps) {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-[#0a183d] via-[#142e5c] to-[#0a183d] flex flex-col items-center justify-center py-8">
      <div className="w-full max-w-xl mx-auto space-y-6">
        {/* Back Button */}
        <button onClick={onBack} className="text-slate-300 text-sm flex items-center gap-2 hover:text-white mb-2">
          <span className="text-lg">←</span> Back to Orders
        </button>

        {/* Order Summary Card */}
        <div className="bg-[#101c3a] rounded-2xl shadow-lg p-6 border border-[#1a2a4d]">
          <div className="flex items-center justify-between mb-2">
            <div className="font-mono text-xs text-slate-300">Order #{order.id}</div>
            <span className="px-3 py-1 rounded-full bg-slate-700 text-xs text-slate-300 border border-slate-600">{order.status.charAt(0).toUpperCase() + order.status.slice(1)}</span>
          </div>
          <div className="flex items-center justify-center gap-8 py-4">
            <div className="text-center">
              <div className="text-2xl font-bold text-white">{order.fromAmount}</div>
              <div className="text-xs text-slate-400">ETH<br/>EVM</div>
            </div>
            ...
            <div className="text-center">
              ...
              <div className="text-xs text-slate-400">ADA<br/>Cardano</div>
            </div>
          </div>
          <div className="grid grid-cols-2 gap-4 text-xs text-slate-400 mt-2">
            <div>
              <div>Created:</div>
              <div className="font-mono text-slate-300">{order.createdAt}</div>
            </div>
            <div>
              <div>Exchange Rate:</div>
              <div className="font-mono text-slate-300">{order.exchangeRate || '-'}</div>
            </div>
            <div>
              <div>Fee:</div>
              <div className="font-mono text-slate-300">{order.fee || '-'}</div>
            </div>
            <div>
              <div>Estimated Time:</div>
              <div className="font-mono text-slate-300">{order.estimatedTime || '-'}</div>
            </div>
          </div>
        </div>
        ...
        <div className="bg-[#101c3a] rounded-2xl shadow-lg p-6 border border-[#1a2a4d]">
          <div className="text-slate-300 font-semibold mb-2">Transaction Details</div>
          <div className="space-y-2 text-xs">
            <div className="flex items-center gap-2">
              <span className="text-slate-400">From Address:</span>
              <span className="font-mono text-slate-300">{order.fromAddress?.slice(0,6)}...{order.fromAddress?.slice(-4)}</span>
              <button onClick={() => copyToClipboard(order.fromAddress)} className="text-slate-400 hover:text-white text-xs">📋</button>
            </div>
            <div className="flex items-center gap-2">
              <span className="text-slate-400">To Address:</span>
              <span className="font-mono text-slate-300">{order.toAddress?.slice(0,6)}...{order.toAddress?.slice(-4)}</span>
              <button onClick={() => copyToClipboard(order.toAddress)} className="text-slate-400 hover:text-white text-xs">📋</button>
            </div>
            <div className="flex items-center gap-2">
              <span className="text-slate-400">Transaction Hash:</span>
              <span className="font-mono text-slate-300">{order.txHash ? `${order.txHash.slice(0,6)}...${order.txHash.slice(-4)}` : '-'}</span>
              {order.txHash && <a href={order.fromChain === "Cardano" ? `https://preprod.cardanoscan.io/transaction/${order.txHash}` : `https://sepolia.etherscan.io/tx/${order.txHash}`} target="_blank" rel="noopener noreferrer" className="text-blue-400 hover:underline">View</a>}
            </div>
          </div>
        </div>
        ...
        <div className="bg-[#101c3a] rounded-2xl shadow-lg p-6 border border-[#1a2a4d]">
          <div className="text-slate-300 font-semibold mb-4">Progress</div>
          <div className="flex flex-col gap-4">
            {order.steps?.map((step, idx) => (
              <div key={step.step} className="flex items-center gap-3">
                <div className={`w-6 h-6 flex items-center justify-center rounded-full border-2 ${step.status === 'completed' ? 'bg-green-500 border-green-500' : step.status === 'pending' ? 'bg-yellow-400 border-yellow-400' : 'bg-slate-600 border-slate-600'}`}>
                  {step.status === 'completed' ? <span className="text-white font-bold">{step.step}</span> : <span className="text-slate-900 font-bold">{step.step}</span>}
                </div>
                <div className="flex flex-col">
                  <span className={`font-semibold text-sm ${step.status === 'completed' ? 'text-green-300' : step.status === 'pending' ? 'text-yellow-300' : 'text-slate-300'}`}>{step.description}</span>
                  {step.href && <a href={step.href} target="_blank" rel="noopener noreferrer" className="text-xs text-blue-400 hover:underline">Verify on chain</a>}
                  {step.status === 'pending' && step.action && (
                    <button onClick={() => handleStepAction(step.action)} className="mt-1 px-3 py-1 rounded bg-blue-600 text-white text-xs font-medium hover:bg-blue-700">Complete Step</button>
                  )}
                </div>
              </div>
            ))}
